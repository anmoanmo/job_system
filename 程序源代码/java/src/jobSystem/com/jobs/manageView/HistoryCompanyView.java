	...
	public void createWindow() {
		...
		regainBtn.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if(rowIndex == -1) {
					JOptionPane.showMessageDialog(HistoryCompanyView.this, "请先选中要恢复的信息！","提示",JOptionPane.WARNING_MESSAGE);
				}else {
					int flag = JOptionPane.showConfirmDialog(HistoryCompanyView.this, "确认要恢复选中的信息吗？","提示",JOptionPane.YES_NO_OPTION);
					if(flag == 0) {
						...
						if(manageDao.regainCompany(companyNumber)) {
							...
							refreshTableModel();
							JOptionPane.showMessageDialog(HistoryCompanyView.this, "恢复成功！","提示",JOptionPane.INFORMATION_MESSAGE);
						}else {
							JOptionPane.showMessageDialog(HistoryCompanyView.this, "恢复失败！","提示",JOptionPane.ERROR_MESSAGE);
						}
					}
				}
			}
		...
		deleteBtn.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				int rowIndex = table.getSelectedRow();
				if(rowIndex == -1) {
					JOptionPane.showMessageDialog(HistoryCompanyView.this, "请先选中要删除的信息！","提示",JOptionPane.WARNING_MESSAGE);
				}else {
					int flag = JOptionPane.showConfirmDialog(HistoryCompanyView.this, "确认要删除选中的信息吗？删除后将不可恢复！","提示",JOptionPane.YES_NO_OPTION);
					if(flag == 0) {
						int companyNumber = ((Integer)table.getModel().getValueAt(rowIndex, 0)).intValue();
						String companyId = ((String)table.getModel().getValueAt(rowIndex, 2));
						ManageDao manageDao = new ManageDao();
						if(manageDao.deleteCompany(companyNumber)) {
							...
							refreshTableModel();
							JOptionPane.showMessageDialog(HistoryCompanyView.this, "删除成功！","提示",JOptionPane.INFORMATION_MESSAGE);
						}else {
							JOptionPane.showMessageDialog(HistoryCompanyView.this, "删除失败！","提示",JOptionPane.ERROR_MESSAGE);
						}
					}
				}
			}
